...
class StudentActivity : AppCompatActivity() {
    ...
    private val studentId: Long by lazy { intent?.getLongExtra(EXTRA_STUDENT_ID, 0) ?: 0 }
    ...
    private fun loadFragment() {
        if (supportFragmentManager.findFragmentByTag(TAG_STUDENT_FRAGMENT) == null) {
            replaceFragment(R.id.flContent, StudentFragment.newInstance(studentId), TAG_STUDENT_FRAGMENT)
        }
    }
